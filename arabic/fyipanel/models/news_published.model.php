...
class news_publishedModel
{
    ...
    public static function newsStartCountVideos3($nb)
    {
        include("fyipanel/views/connect.php");
        $tab = array();
        $emaill = "";
...
//azaz
    ...
    public function nbrNewsBySource($src)
    {
        ...
        if ($emaill != "") {
            ...
            if (isset($_COOKIE['arabic_sources'])) { //cookie
                $data = json_decode($_COOKIE['arabic_sources'], true);
                if (count($data) > 0) {
...
and media!='' group by title UNION ALL select id FROM news_published where  type='" . $src . "' and media!='' and media not like '%.pdf' and media not like '%.PDF' )x");
    ...
    public function nbrBreakingNews()
    {
        ...
        if ($emaill != "") {
            $requete = $con->prepare("select count(*) from (select r.id from rss r,rss_sources s where r.favorite=s.id and title like '%عاجل%' and r.favorite not in (select id from user_sources where 
email='$emaill') group by
 title )x");
            ...
            if (isset($_COOKIE['arabic_sources'])) { //cookie
                $data = json_decode($_COOKIE['arabic_sources'], true);
                if (count($data) > 0) {
                    $requete = $con->prepare("select count(*) from (select r.id from rss r,rss_sources s where r.favorite=s.id and title like '%عاجل%' and favorite NOT IN ( '" . implode("', '", $data) . "' )  group by title )x");
                } else {
                    $requete = $con->prepare("select count(*) from (select r.id from rss r,rss_sources s where r.favorite=s.id  and title like '%عاجل%' group by title )x");
                }
            } else {//no cookie
                $requete = $con->prepare("select count(*) from (select r.id from rss r,rss_sources s where r.favorite=s.id  and title like '%عاجل%' group by title )x");
            }
...
//zzz2 
    ...
    public function getspecialfeedsBreakingNews($start, $nb)
    {
        ...
        if ($emaill != "") {
            $query = $con->query("select r.id,`title`,`media`,`link`,`pubDate`,description,`Source` from rss r, rss_sources s  where r.favorite=s.id and title like '%عاجل%' and favorite not in (select id from user_sources where 
email='$emaill') ORDER BY UNIX_TIMESTAMP(pubDate) desc LIMIT " . $start . "," . $nb . "");
            ...
            if (isset($_COOKIE['arabic_sources'])) {//cookie
                $data = json_decode($_COOKIE['arabic_sources'], true);
                if (count($data) > 0) {
                    $query = $con->query("select  r.id,`title`,`media`,`link`,`pubDate`,description,`Source` from rss r, rss_sources s  where r.favorite=s.id and title like '%عاجل%' and  favorite NOT IN ( '" .
                        implode("', '", $data) . "' )  ORDER BY 
UNIX_TIMESTAMP(pubDate) desc LIMIT " . $start . "," . $nb . "");
                } else {
                    $query = $con->query("select r.id,`title`,`media`,`link`,`pubDate`,description,`Source` from rss r, rss_sources s  where r.favorite=s.id and title like '%عاجل%' ORDER BY UNIX_TIMESTAMP(pubDate) desc LIMIT " . $start . "," . $nb . "");
    ...
    public function findnews($id)
    {
        ...
        if ($data = $rqt->fetch()) {
            ...
            $this->right_of_reply_id = $data['right_of_reply_id'];
            return $this;
        } else {
            return null;
    ...
    public function getCategoryReplies($src, $start, $nb)
    {
        ...
        /*stmt = $con->prepare("select *, news_published.id as published_news_id from news_published join rss_right_of_reply where news_published.right_of_reply_id =
        rss_right_of_reply.id and news_published
.right_of_reply_id
 > 0
and news_published.type = '$src' order by news_published.id desc LIMIT " . $start . "," . $nb ."");*/
