...
foreach ($sources as $key =>$val){   
    ...
    foreach(returnTweet($val['name']) as $i){ 
        ...
        $link = ''; 
        if($val['name']=='Alwatan_Live'){
            if(isset($i['entities']['urls'][1]['expanded_url'])){ 
                $link = $i['entities']['urls'][1]['expanded_url'];
            }
        }else{
        ...
        if($q->rowCount() < 1){  
            ...
            $id = $con->lastInsertId();  
             ...
             $n2->sendTopicNotification2($u::source($val['source_name']),$title, 'ar-'.str_replace(' ','__',$val['country']).'-'.str_replace(' ','__',$val['source_name']), $id,$link, 'ar');
            ...
            $fcm->sendWebNoification('ar-'.str_replace(' ','__',$val['country']).'-'.str_replace(' ','__',$val['source_name']), 
                $u::source($val['source_name']).' | '.$title,'FYIPress', $link);
            usleep(250000);
