...
{
    $fav = 0;
    if ($source == 'Sabah.com.tr') $fav = 8;
    if ($source == 'Haberturk.com') $fav = 9;
    if ($source == 'Sozcu.com.tr') $fav = 19;
    if ($source == 'Hurriyet.com.tr') $fav = 22;
    return $fav;
}
                    ...
                    if (api_get_source_status($favorite) != null && api_get_source_status($favorite) == 1) {
                        if ($favorite != 0) {
                            ...
                            if ($date != "" && NbnewsBytitleInLastDate($date, $title, $favorite) == 0 && addOrNot($date)) {
                                ...
                                $id = $con->lastInsertId();
                                $n->sendTopicNotification($name,$title, 'tu-'.$country.'-'.str_replace(' ','__',$name), $id, $link,'tu');
                                  //zzz  
                                ...
                                $fcm->sendWebNoification('tu-'.$country.'-'.str_replace(' ','__',$name), $name.' | '.$title, 'FYIPress', $link);
                                usleep(250000);  
                            }
...
try {
    ...
    fill_rss_tmp();
    $xSource = 'https://newsapi.org/v2/top-headlines?country=tr&category=general&apiKey=978ce545166e433ab2a5d5f57610c194';
    $t = 'News';
    api_withmedia($xSource, $t, 'Turkey');
