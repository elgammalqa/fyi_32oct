...
foreach ($sources as $key =>$val){ 
    ...
    foreach(returnTweet($val['name']) as $i){
        ...
        if($q->rowCount() < 1){
            ...
            $n->sendTopicNotification($val['source_name'],$title, 'ge-'.str_replace(' ','__',$val['country']).'-'.str_replace(' ','__',$val['source_name']), $id, $link, 'ge');
            ...
            $fcm->sendWebNoification('ge-'.str_replace(' ','__',$val['country']).'-'.str_replace(' ','__',$val['source_name']), $val['source_name'].' | '.$title,
                'FYIPress', $link);
