...
require_once('fyipanel/models/news_published.model.php');
  ...
  function sources()  {
        ...
        $tab = array();
        $query = $con->query("select * from rss_sources group by source order by id ");
    ...
    function news($id){  
        include("fyipanel/views/connect.php");
        $tab = array();
        $query = $con->query("select distinct * from rss where favorite='".$id."' LIMIT 1");
        while ($data = $query->fetch()) {
            $tab[] = $data;
        }
        return $tab;
    ...
    foreach ($news as $key => $v) {  
        ...
        $link='test2.php?url='.$url; 
        echo '<pre> <a target="_blank" href="'.$url.'" > '.$url.'</a><br></pre>';
