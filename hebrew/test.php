...
require_once('fyipanel/models/news_published.model.php');
  ...
  function sources()  { 
        ...
        $tab = array();
        $query = $con->query("select * from rss_sources where id  >31 group by source order by id");
    ...
    foreach ($news as $key => $v) {   
        ...
        $link = "http://localhost/fyi8/project/hebrew/iframe2.php?link=".stripslashes($url)."&id=".$v['id'];
