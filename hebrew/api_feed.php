...
{ 
    $fav = 0;
    if ($source == 'Haaretz.co.il') $fav = 2;
    if ($source == 'Globes.co.il') $fav = 5;
    if ($source == 'Israelhayom.co.il') $fav = 8;
    if ($source == 'Mako.co.il') $fav = 10;
    if ($source == 'Calcalist.co.il') $fav = 17;
    if ($source == 'Ynet') $fav = 21;
    if ($source == 'Srugim.co.il') $fav = 28;
    if ($source == 'Bizportal.co.il') $fav = 32;
    if ($source == 'Maariv.co.il') $fav = 33;
    if ($source == 'Walla.co.il') $fav = 34;
    if ($source == 'Medinet.co.il') $fav = 35;
    return $fav;
} 
                    ...
                    if (api_get_source_status($favorite) != null && api_get_source_status($favorite) == 1) {
                        if ($favorite != 0) {
                            ...
                            if ($date != "" && NbnewsBytitleInLastDate($date, $title, $favorite) == 0 && addOrNot($date)) {
                                ...
                                $id = $con->lastInsertId();
                                $n->sendTopicNotification($name,$title, 'he-'.$country.'-'.str_replace(' ','__',$name), $id, $link,'he');
                                ...
                                $fcm->sendWebNoification('he-'.$country.'-'.str_replace(' ','__',$name), $name.' | '.$title, 'FYIPress', $link);
                                usleep(250000);  
                            }
...
try {
    ...
    fill_rss_tmp();
    $xSource = 'https://newsapi.org/v2/top-headlines?country=il&category=general&apiKey=fd87851d8c514ba89ff14cd8aa635176';
    $t = 'News';
    api_withmedia($xSource, $t, 'Israel');
