...
foreach ($sources as $key =>$val){ 
    ...
    foreach(returnTweet($val['name']) as $i){
        ...
        if($val['name'] == 'cnnbrk' || $val['name'] == 'BBCBreaking'){
            $breaking_heading = 'Breaking News: ';
            $title = $breaking_heading.$title;
        ...
        $q = $con->prepare("select count(*) from rss where favorite = '$val[id]' and (link = '$link' or title = '$title') ");
        $q->execute();
        $nbrr=$q->fetchColumn();
        if($nbrr < 1){
            ...
            $n->sendTopicNotification($val['source_name'],$title, 'en-'.str_replace(' ','__',$val['country']).'-'.str_replace(' ','__',$val['source_name']), $id, $link, 'en');
            ...
            $fcm->sendWebNoification('en-'.str_replace(' ','__',$val['country']).'-'.str_replace(' ','__',$val['source_name']), $val['source_name'].' | '.$title,'FYIPress', $link);  
            usleep(250000);  
        }
    }//
    }
