...
{ 
    $fav = 0;
    if ($source == 'Bjnews.com.cn') $fav = 31;
    if ($source == 'Newssc.org') $fav = 32;
    return $fav;
}
                    ...
                    if (api_get_source_status($favorite) != null && api_get_source_status($favorite) == 1) {
                        if ($favorite != 0) {
                            ...
                            if ($date != "" && NbnewsBytitleInLastDate($date, $title, $favorite) == 0 && addOrNot($date)) {
                                ...
                                $id = $con->lastInsertId();
                                $n->sendTopicNotification($name,$title, 'ch-'.$country.'-'.str_replace(' ','__',$name), $id, $link,'ch'); 
                                 //zzz  
                                ...
                                $fcm->sendWebNoification('ch-'.$country.'-'.str_replace(' ','__',$name), $name.' | '.$title, 'FYIPress', $link);
                                usleep(250000);  
                            }
...
try {
    ...
    fill_rss_tmp();
    $xSource = 'https://newsapi.org/v2/top-headlines?country=cn&category=general&apiKey=718bdc31fdf747ea9807ad06ff8d68ad';
    $t = 'News';
    api_withmedia($xSource, $t, 'China');
