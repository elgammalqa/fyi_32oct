...
{
    $fav = 0;
    if ($source == 'Le Monde') $fav = 8;
    if ($source == 'Ouest-france.fr') $fav = 46;
    return $fav;
}
                    ...
                    if (api_get_source_status($favorite) != null && api_get_source_status($favorite) == 1) {
                        if ($favorite != 0) {
                            ...
                            if ($date != "" && NbnewsBytitleInLastDate($date, $title, $favorite) == 0 && addOrNot($date)) {
                                ...
                                $id = $con->lastInsertId();
                                $n->sendTopicNotification($name,$title, 'fr-'.$country.'-'.str_replace(' ','__',$name), $id, $link,'fr');
                                //zzz  
                                ...
                                $fcm->sendWebNoification('fr-'.$country.'-'.str_replace(' ','__',$name), $name.' | '.$title, 'FYIPress', $link);
                                usleep(250000);  
                            }
...
try {
    ...
    fill_rss_tmp();
    $xSource = 'https://newsapi.org/v2/top-headlines?country=fr&category=general&apiKey=4ecc940439d14a619505f5e25f197d5e';
    $t = 'News';
    api_withmedia($xSource, $t, 'France');
