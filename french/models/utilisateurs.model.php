...
class utilisateursModel
{
    ...
    public static function type2($word)
    {
        $source = "";
        if ($word == "News") $source = " Actualités  ";
        else if ($word == "Sports") $source = " Sport ";
        ...
        else if ($word == "General Culture" || $word == "Culture") $source = " Culture ";
        return $source;
    }
    ...
    public static function get_id_rss_source($country, $source, $type)
    {
        ...
        $sql = $con->prepare("SELECT id FROM rss_sources WHERE country='" . $country . "' AND source='" . $src . "' AND type='" . $type . "' ");
        $sql->execute();
        $pass = $sql->fetchColumn();
        return $pass;
    }
