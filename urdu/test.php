...
require_once('fyipanel/models/news_published.model.php');
  ...
  function sources()  { 
        ...
        $tab = array();
        $query = $con->query("select * from rss_sources where id  in 
            (13,21,35,38,39,42,43,44,55,59,60,65,66) group by source order by id");
    ...
    foreach ($news as $key => $v) {   
        ...
        $link = "http://localhost/fyi8/project/urdu/iframe2.php?link=".stripslashes($url)."&id=".$v['id'];
