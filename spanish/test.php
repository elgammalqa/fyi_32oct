...
require_once('fyipanel/models/news_published.model.php');
  ...
  function sources()  { 
        ...
        $tab = array();
        $query = $con->query("select * from rss_sources where id in (62,63,65,67,68) group by source order by id");
    ...
    foreach ($news as $key => $v) {   
        ...
        $link = "http://localhost/fyi8/project/spanish/iframe2.php?link=".stripslashes($url)."&id=".$v['id'];
